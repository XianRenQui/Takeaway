 ...
 * @LastEditors: 张晓兰
 * @LastEditTime: 2019-11-15 15:25:25
 * @version: v1.0.0
 -->
<template>
    <div class="Goods">
        <div class="GoodsDetail_head" :style="{ 'background-image':'url(' + this.Detali.img + ')'}">
            ...
            <div class="Evaluation">
                ...
                <p>{{this.Detali.description}}</p>
                <span class="eva"> &nbsp;|&nbsp; 243条评论 </span>
                <i class="el-icon-arrow-down"></i>
            </div>
        </div>
        <div class="share">
            <h6>{{this.Detali.productname}}</h6>
            <img class="image_a" src="../assets/img/share.png" alt="">
            <p>月售{{this.Detali.Monthlysale}}</p>
            <span class="money">￥{{this.Detali.price}}</span>
...
export default {
    ...
    created() {
        console.log(this.id);
        //从后端获取数据     
        axios.get('goods/selectAll?id='+this.id)
        .then(res=>{ 
            console.log(res.data);
            // var gid = this.$route.query.gid;
         
            this.Detali = res.data[0];
            // for(var i = 0;i<data.length;i++){
            //     // console.log(data)
            //     if(data[i].gid == gid){
            //       that.Detali = data[i];
            //       console.log(Detali.img)
            //     }
            // }
        })
        .catch(err=>{
